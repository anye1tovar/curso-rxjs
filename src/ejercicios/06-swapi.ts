...
(() => {
    ...
    const PEOPLE_ENDPOINT = "/people/66/"
    // Realizar el llamado al URL para obtener a Luke Skywalker
    zip(getRequest(`${SW_API}${PEOPLE_ENDPOINT}`).pipe(
        // Realizar los operadores respectivos aquí
        switchMap(res => getRequest(res.species[0]))
    ), getRequest(`${SW_API}${PEOPLE_ENDPOINT}`)
        // NO TOCAR el subscribe ni modificarlo ==
    ).pipe(
        map(([specie, character]) => ({ specie, character }))
    ).subscribe(console.log)           // ==
    // =======================================

    // SOLUCION CURSO
    getRequest(`${SW_API}${PEOPLE_ENDPOINT}`).pipe(
        // Realizar los operadores respectivos aquí
        switchMap(res => zip(of(res), getRequest(res.species[0]))),
        map(([character2, specie2]) => ({ character2, specie2 }))
    ).subscribe(console.log)
})();
